...
Value checkClueConsistency(const CallbackInfo& info) {
  ...
  auto name_list = makeStringList(env, info[0].As<Array>());
  // arg1:
  auto generate_max = info[1].As<Number>().Int32Value();
  // arg2:
  auto load_max = info[2].As<Number>().Int32Value();
  // arg3:
  auto version = info[3].As<Number>().Int32Value();
  // --
  bool result{true};
  switch (version) {
  case 1:
    ...
    break;
  case 2:
    components::consistency_check(std::move(name_list), generate_max, load_max);
    break;
  default:
    assert(false);
  }
