...
namespace hax _GLIBCXX_VISIBILITY(default)
{
...
#else
          throw std::overflow_error("bitset");
#endif
      return _M_w[0];
    }
...
_GLIBCXX_BEGIN_NAMESPACE_VERSION
  ...
  template<size_t _Nb>
    struct hash<hax::bitset<_Nb>>
    : public __hash_base<size_t, hax::bitset<_Nb>>
    {
      size_t
      operator()(const hax::bitset<_Nb>& __b) const noexcept
      {
  ...
  template<>
    struct hash<hax::bitset<0>>
    : public __hash_base<size_t, hax::bitset<0>>
    {
      size_t
      operator()(const hax::bitset<0>&) const noexcept
      { return 0; }
    };
